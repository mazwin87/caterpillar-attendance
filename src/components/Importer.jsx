...
const VALID_METHODS = ['Cash', 'Bank Transfer', 'Cheque']
const VALID_AGE_GROUPS = ['3-6months', '7-12months', '1year', '2year', '3year', '4year', '5year', '6year']
    ...
    reader.onload = (ev) => {
      ...
      const errors = []
        rows.forEach((r, i) => {
            ...
            if (r.age_group && !VALID_AGE_GROUPS.includes(r.age_group.trim())) {
              errors.push(`Row ${rowNum}: age_group "${r.age_group}" is invalid — must be one of: ${VALID_AGE_GROUPS.join(', ')}`)
            }
            if (r.date_of_birth && !/^\d{4}-\d{2}-\d{2}$/.test(r.date_of_birth.trim())) {
                errors.push(`Row ${rowNum}: date_of_birth must be YYYY-MM-DD format`)
            }
            if (r.fee_month && !VALID_MONTHS.includes(r.fee_month.trim())) {
                errors.push(`Row ${rowNum}: fee_month "${r.fee_month}" is invalid — use full month name e.g. April`)
            }
            if (r.fee_payment_method && !VALID_METHODS.includes(r.fee_payment_method.trim())) {
