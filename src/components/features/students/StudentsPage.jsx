...
export default function StudentsPage({ session }) {
  ...
  const filtered = hook.students.filter(s => {
    ...
    const matchAttendance = !filterAttendance ||
      hook.attendanceMap[s.id] === filterAttendance ||
      (filterAttendance === 'ABSENT' && !hook.attendanceMap[s.id])
    return matchSearch && matchBranch && matchGroup && matchTelegram && matchAttendance
  })

  async function handleAdd(formData) {
    await hook.add(formData)
  ...
  async function handleEdit(formData) {
    await hook.update(editStudent.id, formData)
  ...
  async function handleBatchPrint() {
    ...
    try {
      await hook.batchPrintQR(printBranch)
    ...
    <div style={{ minHeight: '100%', background: 'var(--bg)', paddingBottom: 100 }}
      ...
      <div style={{ background: 'var(--surface)', borderBottom: '0.5px solid var(--border)', padding: '52px 20px 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>{filtered.length} of {hook.students.length} students</div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          ...
          >
            ...
            {hook.branches.map(b => <option key={b.id} value={b.id}>{b.slug}</option>)}
          </Select>
        </div>
      ...
      <div style={{ padding: '12px 16px' }}>
        {hook.loading ? (
          ...
          <StudentList
            students={filtered}
            attendanceMap={hook.attendanceMap}
            openMenu={openMenu}
      ...
      {showAddForm && (
        <StudentForm
          ...
          onBranchSelect={hook.getAutoStudentNo}
        />
      )}
        ...
        <Modal onClose={() => setShowPrintModal(false)}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Select value={printBranch} onChange={e => setPrintBranch(e.target.value)}>
              ...
              {hook.branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}
            </Select>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <Button onClick={handleBatchPrint} disabled={hook.printingQR || !printBranch} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                🖨️ {hook.printingQR ? 'Generating...' : 'Print'}
              </Button>
            </div>
          </div>
        </Modal>
