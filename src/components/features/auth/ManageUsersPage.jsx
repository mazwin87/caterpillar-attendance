...
export default function ManageUsersPage({ session }) {
  ...
  const hook = useManageUsers(session)
  ...
  async function handleReset(e) {
    ...
    try {
      await hook.resetPassword(resetUser.id, newPass)
      ...
      {resetUser && (
        <Modal onClose={closeReset} variant="center">
          ...
          <form onSubmit={handleReset} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <Button type="submit" disabled={hook.saving} style={{ padding: 14, fontSize: 15 }}>
              {hook.saving ? 'Saving...' : 'Reset Password'}
            </Button>
          </form>
        </Modal>
      )}
